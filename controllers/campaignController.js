...
async function sendViaBrevo({ to, toName, subject, htmlBody, fromEmail, fromName, replyTo, attachment, apiKey, companyAddress, campaignTag }) {
...
async function sendEmails(campaign, filter, cfg, apiKeys, companyAddress, blastLog) {
  ...
  while (true) {
    ...
    const leads = await Lead.find(filter).select('_id email name firstName company').skip(skip).limit(BATCH).lean();
    ...
    for (const lead of leads) {
      ...
      try {
        ...
        await sendViaBrevo({ to: lead.email, toName: pName, subject: subj, htmlBody: html, fromEmail, fromName, replyTo: fromEmail, attachment, apiKey: activeAcc.key, companyAddress: cfg.address, campaignTag: `camp-${campaign._id}` });
        ...
        // Mark the lead as not_sent so it shows in the Leads → Not Sent filter
        await Lead.findByIdAndUpdate(lead._id, {
          status:         'not_sent',
          lastBlastError: reason.slice(0, 300),
          lastBlastAt:    new Date(),
        }).catch(() => {});
